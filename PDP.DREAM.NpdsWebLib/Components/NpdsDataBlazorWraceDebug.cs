...
public abstract partial class WraceBlazorComponentBase : ComponentBase
{
  ...
  // PDP Atlas Data Context (PCDC) for Atlas data repositories of data/metadata records
  ...
  protected void DebugWraceRazorPage(string rzrHandlerName = ESS, string rzrCntrllrName = ESS)
  {
  ...
  {
    NPDSCW.CatchNullObject(npdswk, methodName, className);
  }

  protected void WraceAddErrors(string error)
  {
    // for Blazor, use EditForm component and EditContext class
