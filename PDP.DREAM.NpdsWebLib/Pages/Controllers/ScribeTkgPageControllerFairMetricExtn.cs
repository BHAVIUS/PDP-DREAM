...
public partial class ScribeTkgPageController
{
    ...
    Guid recordGuid, bool isLimited = false)
  {
    ...
    {
      dsResult = NPDSCW.ScribeDR.ListEditableFairMetricExtns(recordGuid, isLimited)
    ...
    Guid recordGuid, bool isLimited = false)
  {
    ...
    if (ModelState.IsValid) { fgr = NPDSCW.ScribeDR.EditFairMetricExtn(fgr); }
  ...
  {
    NPDSCW.OpenScribeConnection(); // use PSDC
    FairMetricExtnUxm? fgr = NPDSCW.ScribeDR.GetEditableFairMetricExtnByFGuid(fgroupGuid);
    if (fgr?.RRFgroupGuid == fgroupGuid)
    { fgr = NPDSCW.ScribeDR.CheckFairMetricExtn(fgr); }
    fgr.NdisElemId = eidFairMetricExtnStatus;
    DataSourceResult dsResult = (new[] { fgr }).ToDataSourceResult(dsRequest, ModelState);
    var jsonData = new JsonResult(dsResult, QebKendoJsonOptions);
    NPDSCW.CloseScribeConnection();
