...
public class ScribeServiceAtlasServices : ScribeTkgPageController
{
  private const string rzrClass = nameof(ScribeServiceAtlasServices);
  public ScribeServiceAtlasServices() : base() { }

  // ScribeService Atlas Services
  public const string pageName = DepScribeServiceAtlasServices;
  ...
  public IActionResult OnGet(string serviceType, string serviceTag, string entityType, string resrepFormat)
  {
    ...
    if (string.IsNullOrEmpty(entityType)) { entityType = NPDSCD.EntityTypeNpdsServices.EName; }
    if (string.IsNullOrEmpty(resrepFormat)) { resrepFormat = NPDSCD.ResrepFormatAtlas.EName; }
