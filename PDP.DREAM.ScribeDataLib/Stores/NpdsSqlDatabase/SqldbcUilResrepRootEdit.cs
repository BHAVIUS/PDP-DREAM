...
namespace PDP.DREAM.ScribeDataLib.Stores
{
  ...
  public partial class ScribeDbsqlContext
  {
    ...
    public NexusResrepEditModel EditResrepRoot(NexusResrepEditModel editObj, bool byStorProc = true)
    {
      ...
      {
        // allow editable values different from current request settings only for admin authors
        editObj.RecordDiristryGuid ??= PRC.DiristryGuid; // Nexus
        editObj.RecordRegistryGuid ??= PRC.RegistryGuid; // PORTAL
        editObj.RecordDirectoryGuid ??= PRC.DirectoryGuid; // DOORS
        editObj.RecordRegistrarGuid ??= PRC.RegistrarGuid; // Scribe
      }
      else
      {
        // assign values from current request settings for non-admin authors
        editObj.RecordDiristryGuid = PRC.DiristryGuid; // Nexus
        editObj.RecordRegistryGuid = PRC.RegistryGuid; // PORTAL
        editObj.RecordDirectoryGuid = PRC.DirectoryGuid; // DOORS
        editObj.RecordRegistrarGuid = PRC.RegistrarGuid; // Scribe
      ...
      // ATTN: current check scheme only resets diristry to registry
      // if registry = directory and if diristry invalid/empty in order to assure consistency of current scheme 
