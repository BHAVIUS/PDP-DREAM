...
public partial class NexusDbsqlContext // : INpdsDataService
{
  ...
  public NpdsResrepXmlRoot? GetResrepsXmlMessage()
  {
    ...
    if ((QURC == null) || string.IsNullOrEmpty(QURC.ServiceType.ToString()))
    {
      ResrepListMel = null;
    }
    else
    {
      var rrStems = ListStorableNexusStemsWithFacets();
      if (rrStems != null)
      {
        ResrepListMel = CreateResrepListXml(rrStems);
        QURC.NexusRecords = ResrepListMel;
  ...
  public NpdsResrepList CreateResrepListXml(IEnumerable<INexusResrepStem> rrlist)
  {

    try
    {
      ResrepListMel = new NpdsResrepList(PdpAppConst.NpdsFieldRule.Required);
      foreach (INexusResrepStem rr in rrlist)
      {
    ...
    {
      ResrepListMel = null;
      QURC.ResponseNote = ex.Message;
    }
    return ResrepListMel;
  }

  public NpdsResrepItem CreateResrepItemXml(INexusResrepStem rr)
  {
    // Data Transfer Object for Resource Representation
    var rrFormat = qebUserRestCntxt.ResrepFormat;
    ResrepItemMel = new NpdsResrepItem(rrFormat, rr.RecordGuidKey);
