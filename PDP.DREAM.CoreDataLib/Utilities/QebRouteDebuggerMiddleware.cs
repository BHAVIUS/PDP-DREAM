...
public class QebRouteDebuggerMiddleware
{
  ...
  public QebRouteDebuggerMiddleware(RequestDelegate theDelegate, IHttpContextAccessor theAccessor, EndpointDataSource theSource)
  {
  ...
  private string RouteHelpInformation(IList<IRouter> theRouters, bool allHelp = false)
  {
    ...
    foreach (Microsoft.AspNetCore.Routing.Route r in theRouters)
    {
      ...
      if (allHelp || !onlyHelpWhenDebug)
      {
        ...
        string rName = r.Name;
        string rExamples = FormatDataTokenValue(r.GetMvcRouteDataTokenStringArray(QebHttpRouteExtensions.RouteExamplesKey), linbrksep, baseUrl);
