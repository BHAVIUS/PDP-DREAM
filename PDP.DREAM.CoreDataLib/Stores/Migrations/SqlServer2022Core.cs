...
public static class SqlServer2022Core
{
  public static bool CheckDatabase(CoreDbsqlContext? dbCntxt)
  {
  ...
  public static bool CreateDatabase(string dbCnstr, IFileProvider fileProv)
  {
    ...
    CoreDbsqlContext? dbCntxt = null;

    try
    {
      dbCntxt = new CoreDbsqlContext();
    }
    ...
    if (!dbCntxt.Database.CanConnect())
    {
      ...
      var fileInfo = fileProv.GetFileInfo(@"\Migrations\CreatePdpCore10Gangkhar.sql");
      var sqlScript = File.ReadAllText(fileInfo.PhysicalPath, Encoding.UTF8);
      var mstrDbconstr = NPDSSD.CoreDbconstr
        .Replace("PdpCore10Gangkhar", "master");
