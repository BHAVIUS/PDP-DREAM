...
{
  public EntityLabelUxm EditEntityLabel(EntityLabelUxm editObj, bool byStorProc = true)
  {
    var errMsg = ESS;
    ...
    CoreEntityLabel storObj;
    if (isNewRecord)
    {
      // insert new record
      fgroupGuid = PdpNewGuid();
      storObj = new CoreEntityLabel()
      {
    ...
    storObj.ServiceTypeCode = editObj.ServiceTypeCode;
    ...
    if (byStorProc)
    {
      var errCod = ScribeEntityLabelEdit(
         ...
         null, null, null, null,
         storObj.ServiceTypeCode, storObj.TagToken, storObj.LabelUri,
    ...
    {
      if (isNewRecord) { this.CoreEntityLabels.Add(storObj); }
      errMsg = StoreChanges();
    }
  ...
  public EntityLabelUxm DeleteEntityLabel(EntityLabelUxm editObj, bool byStorProc = true)
  {
    var errMsg = ESS;
    var recordName = editObj.ItemXnam;
    var recordIndex = editObj.HasIndex;
    ...
    if (!isNewRecord) // delete existing record
    {
      ...
      {
        this.CoreEntityLabels.Attach(storObj);
        this.CoreEntityLabels.Remove(storObj);
        errMsg = StoreChanges();
      }
      // refresh the edit object
      editObj = GetEditableEntityLabelByKey(fgroupGuid);
