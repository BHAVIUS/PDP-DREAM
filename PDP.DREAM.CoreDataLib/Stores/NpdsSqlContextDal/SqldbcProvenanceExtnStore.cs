...
{
  public ProvenanceExtnUxm EditProvenance(ProvenanceExtnUxm editObj, bool byStorProc = true)
  {
    var errMsg = ESS;
    ...
    DoorsProvenanceExtn storObj;
    if (isNewRecord)
    {
      // insert new record
      fgroupGuid = PdpNewGuid();
      storObj = new DoorsProvenanceExtn()
      {
    ...
    else
    {
      ...
      storObj = GetStorableProvenanceExtnByKey(fgroupGuid);
    ...
    storObj.Provenance = editObj.Provenance ?? ESS;
    ...
    {
      if (isNewRecord) { this.DoorsProvenanceExtns.Add(storObj); }
      errMsg = StoreChanges();
    }
  ...
  public ProvenanceExtnUxm DeleteProvenance(ProvenanceExtnUxm editObj, bool byStorProc = true)
  {
    var errMsg = ESS;
    var recordName = editObj.ItemXnam;
    var recordIndex = editObj.HasIndex;
    ...
    {
      var storObj = GetStorableProvenanceExtnByKey(fgroupGuid);
      ...
      {
        this.DoorsProvenanceExtns.Attach(storObj);
        this.DoorsProvenanceExtns.Remove(storObj);
        errMsg = StoreChanges();
      }
      // refresh the edit object
      editObj = GetEditableProvenanceExtnByKey(fgroupGuid);
