...
namespace PDP.DREAM.CoreDataLib;
...
public class Program
{
  public static void Main(string[] args)
  {
    ...
    var dbConstr = NpdsServiceDefaults.Values.NpdsCoreDbconstr;
    var dbIsValid = SqlServer2019Core.CheckConnection(dbConstr);
    if (dbIsValid)
    {
      dbIsValid = SqlServer2019Core.CheckDatabase(dbConstr);
    }
    else // (if allow dbCreation)
    {
      var coreCrfp = pdwEnvir.ContentRootFileProvider;
      dbIsValid = SqlServer2019Core.CreateDatabase(dbConstr, coreCrfp);
    }
