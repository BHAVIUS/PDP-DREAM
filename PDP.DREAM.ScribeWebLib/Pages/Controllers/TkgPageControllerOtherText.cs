...
public partial class TkgsPageController
{
    ...
    OtherTextEditModel fgr, Guid recordGuid, bool isLimited = false)
  {
    ...
    fgr.PdpStatusElement = eidOtherTextStatus;
    ...
    Guid recordGuid, bool isLimited = false)
  {
    ...
    { fgr = PSDC.CheckOtherText(fgr); fgr.PdpStatusElement = eidOtherTextStatus; }
    DataSourceResult dsResult = (new[] { fgr }).ToDataSourceResult(dsRequest, ModelState);
    var jsonData = new JsonResult(dsResult, QebKendoJsonOptions);
    CloseScribeConnection();
    return jsonData;
    ...
    Guid recordGuid, bool isLimited = false)
  {
    ...
    OtherTextEditModel? fgr = PSDC.GetEditableOtherTextByKey(recordGuid);
    if (fgr?.RRFgroupGuid == recordGuid)
    { fgr = PSDC.ReseqOtherText(fgr); fgr.PdpStatusElement = eidOtherTextStatus; }
    DataSourceResult dsResult = (new[] { fgr }).ToDataSourceResult(dsRequest, ModelState);
    var jsonData = new JsonResult(dsResult, QebKendoJsonOptions);
    CloseScribeConnection();
    return jsonData;
