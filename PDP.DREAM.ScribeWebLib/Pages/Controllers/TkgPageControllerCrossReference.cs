...
public partial class TkgsPageController
{
    ...
    CrossReferenceEditModel fgr, Guid recordGuid, bool isLimited = false)
  {
    ...
    fgr.PdpStatusElement = eidCrossReferenceStatus;
    ...
    Guid recordGuid, bool isLimited = false)
  {
    ...
    { fgr = PSDC.CheckCrossReference(fgr); fgr.PdpStatusElement = eidCrossReferenceStatus; }
    DataSourceResult dsResult = (new[] { fgr }).ToDataSourceResult(dsRequest, ModelState);
    var jsonData = new JsonResult(dsResult, QebKendoJsonOptions);
    CloseScribeConnection();
    return jsonData;
    ...
    Guid recordGuid, bool isLimited = false)
  {
    ...
    CrossReferenceEditModel? fgr = PSDC.GetEditableCrossReferenceByKey(recordGuid);
    if (fgr?.RRFgroupGuid == recordGuid)
    { fgr = PSDC.ReseqCrossReference(fgr); fgr.PdpStatusElement = eidCrossReferenceStatus; }
    DataSourceResult dsResult = (new[] { fgr }).ToDataSourceResult(dsRequest, ModelState);
    var jsonData = new JsonResult(dsResult, QebKendoJsonOptions);
    CloseScribeConnection();
    return jsonData;
