...
public class ScribeRestApiController : ScribeDataRazorViewControllerBase
{
  ...
  public override void OnActionExecuting(ActionExecutingContext oaeCntxt)
  {
    ...
    if (!isValid) { oaeCntxt.Result = Redirect(DepqAnonModeAccessDenied); }
  }
  ...
  {
    var searchFilter = NPDSCD.SearchFilterDiristry.ToString();
    NPDSCW.ParseNpdsSelectFilter(searchFilter, serviceTag, serviceType, entityTag, entityVersion);
    var msgScribe = ScribeRestApiMessage();
    return msgScribe;
  }
  ...
  protected IActionResult ScribeRestApiMessage(bool xsdValidate = false)
  {
    ...
    var rrRoots = NPDSCW.PSDC.ListStorableResrepRootsWithFacets();
    // stage 2 check of vocabulary service
    if (rrRoots?.Count == 0)
    {
      NPDSCW.ResetScribeRepository(false, false, NPDSCD.VocabDbconstr);
      rrRoots = NPDSCW.PSDC.ListStorableResrepRootsWithFacets();
    }
    // generate XML message from item list
    var rrListXml = NPDSCW.PSDC.CreateResrepListXml(rrRoots);
    // PRC.ResponseAnswer = rrListXml; // alternative format response
    NPDSCW.NexusRecords = rrListXml;
