...
public abstract partial class WraceRazorViewControllerBase : Controller
{
  ...
  private const string rzrClass = nameof(WraceRazorViewControllerBase);
  ...
  protected void DebugRazorView(string rzrHandlerName = "", string rzrCntrllrName = "")
  {
  ...
  protected void WraceDevTest()
  {
    ...
    // test-dev-debug from QEB User Agent Session (prefix qeb)
    var qebUsrGuid = WRACE.QebiUserGuid;
    if (PdpGuid.IsInvalidGuid(qebUsrGuid)) { WRACE.ServiceError += "QebUserGuid is null, empty, or invalid"; }
    var qebAgtGuid = WRACE.NpdsAgentGuid;
    if (PdpGuid.IsInvalidGuid(qebAgtGuid)) { WRACE.ServiceError += "QebAgentGuid is null, empty, or invalid"; }
    var qebInfGuid = WRACE.NpdsAgentInfosetGuid;
    if (PdpGuid.IsInvalidGuid(qebInfGuid)) { WRACE.ServiceError += "QebAgentInfosetGuid is null, empty, or invalid"; }
    var qebSsnGuid = WRACE.CiaamSessionGuid;
    if (PdpGuid.IsInvalidGuid(qebSsnGuid)) { WRACE.ServiceError += "QebAgentSessionGuid is null, empty, or invalid"; }
