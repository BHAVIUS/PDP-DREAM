...
using PDP.DREAM.ScribeDataLib.Migrations;
...
public class Program
{
  public static void Main(string[] args)
  {
    ...
    // check create Scribe database
    dbIsValid = QebSql.CheckConnection(NPDSSD.ScribeDbconstr);
    using (var dbContxt = new ScribeDbsqlContext())
    {
      if (dbIsValid)
      {
        dbIsValid = SqlServer2022Scribe.CheckDatabase(dbContxt);
      }
      else if (PDPSS.AppUseDevtestFeature) // (if allow dbCreation)
      {
        var coreCrfp = pdwEnvir.ContentRootFileProvider;
        dbIsValid = SqlServer2022Scribe.CreateDatabase(NPDSSD.ScribeDbconstr, coreCrfp);
      }
    }
    if (!dbIsValid) { throw new Exception(PDPCC.PdpCodeErrmsg); }
    ...
    try
    {
      ...
      // pdwServices.AddDbContext<ScribeDbsqlContext>();
