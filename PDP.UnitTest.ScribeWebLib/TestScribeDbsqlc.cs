...
public class TestScribeDbsqlc : PdpSiteTestBase
{
  private readonly Type typCodeClass = typeof(TestScribeDbsqlc);
  private readonly string namCodeClass = nameof(TestScribeDbsqlc);
  ...
  [Fact]
  public void CreateScribeDbsqlContext()
  {
    bool created = false;
    string dbstat = "Open";
    ...
    try
    {
      using (var dbc = new ScribeDbsqlContext())
      {
        dbc.DbsqlConnect();
        var dbcnctn = dbc.DbsqlCnctn;
    ...
    Assert.True(created);
    Assert.Equal(NPDSCD.ScribeDbconstr, dbcs2);
    Assert.Equal(dbstat, dbstat2);
  }


  [Fact]
  public void CreateScribeNlmmesh()
  {
    bool created = false;
    ...
    try
    {
      ...
      using (var dbc = new ScribeDbsqlContext(npdscp))
      {
        dbc.DbsqlConnect();
        var dbcnctn = dbc.DbsqlCnctn;
        dbcs2 = dbcnctn.ConnectionString;
  ...
  [Fact]
  public void GetCountFromScribeNlmmesh()
  {
    string dbstat = "Open", dbname2 = "", dbcs2 = "", dbstat2 = "";
    ...
    try
    {
      ...
      using (var dbc = new ScribeDbsqlContext(npdscp))
      {
        ...
        dbc.NPDSDC.DebugNpdsSelectFilter(nameof(GetCountFromScribeNlmmesh), namCodeClass);
